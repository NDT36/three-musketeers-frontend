...
const UpdateTransactionPage: FC<IProps> = (props) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { error, result } = await callApi(fetchDetailTransaction(String(id)));
      if (error) reactAlert.error(t(`error.${error}`));
      ...
      if (!error) {
        ...
        if (!result?.data?.sourceId) {
          onChoseOtherSource(true);
        } else {
          const source = sources?.find((item) => item._id === result?.data?.sourceId);
          if (source) formik.setFieldValue('source', source);
          else onChoseOtherSource(true);
        }
        ...
        if (!result?.data?.categoryId) {
          ...
          const category = categories?.find((item) => item._id === result?.data?.categoryId);
          if (category) {
  ...
  return (
    <div className="h-full flex flex-col pb-20">
      ...
      <SubPageWrapper routeGoBack={-1} title="" isDisableBtnBack={false}>
        <div className="font-bold text-center text-4xl px-2">Update Transaction</div>
        <br />
