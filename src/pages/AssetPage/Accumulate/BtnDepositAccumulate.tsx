...
const BtnDepositAccumulate: FC<IProps> = (props) => {
  ...
  const validationSchema: Yup.SchemaOf<IDepositAccumulte> = Yup.object().shape({
    money: Yup.number()
  ...
  const onSubmit = async (values: IDepositAccumulte) => {
    setLoading(true);
  ...
  const defaulDescription = 'Gửi tiền vào tích lũy "{A}"';

  const formik = useFormik<IDepositAccumulte>({
    initialValues: {
      money: '',
      otherMoney: '',
      actionAt: '',
...
export default BtnDepositAccumulate;
